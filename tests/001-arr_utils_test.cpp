...
TEST_CASE( "Overlay arrangements case with one arrangement inside another, partly sharing sides" ) {
    ...
    auto arr1 = create_arrangement<Arrangement_2>(points1), arr2 = create_arrangement<Arrangement_2>(points2);
    std::vector<Arrangement_2> v_a = { arr1, arr2 };
    auto overlay_arr = overlay_arrangements<Arrangement_2>(Arrangement_2(), v_a);
    REQUIRE(overlay_arr.number_of_faces() == 3);
    REQUIRE(overlay_arr.number_of_edges() == 8);
    for (auto vit = overlay_arr.vertices_begin(); vit != overlay_arr.vertices_end(); ++vit) {
        if (vit->point() == Kernel::Point_2(0, 2) || vit->point() == Kernel::Point_2(2, 0)) {
            REQUIRE(vit->degree() == 3);
        } else REQUIRE(vit->degree() == 2);
    }
}
