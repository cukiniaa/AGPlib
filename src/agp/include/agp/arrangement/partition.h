/*! \file partition.h
 * \brief Provides functions for partitioning a polygon - triangulation and fracture to convex components.
 *
...
/*! \fn void to_convex_components (const std::vector<Kernel::Point_2> &vertices, std::list<Partition_Polygon_2> &output)
 * \brief Divides a polygon into convex components.
 * @param vertices - vector of vertices representing a polygon
 * @param output - list of convex components of the given polygon
 */
void to_convex_components (const std::vector<Kernel::Point_2> &vertices, std::list<Partition_Polygon_2> &output);

/*! \fn std::list<Kernel::Point_2> arr_discretization (const Arrangement_2 &arr)
 *  \brief Discretizes a simple polygon given as an instance of the Arrangement_2 class from CGAL.
 *  It divides the polygon into convex components and returns their centroids.
 ...
 * \brief Triangulates a given polygon using a build-in method from CGAL. Vertices are expected to be in counterclockwise orientation.
...
Arrangement triangulate (const std::vector<Kernel::Point_2> &vertices) {
    ...
    for(const auto &part: partition_polys) {
        ...
        for (auto face = t.finite_faces_begin(); face != t.finite_faces_end(); ++face) {
            std::vector<Kernel::Point_2> vertices = {face->vertex(0)->point(), face->vertex(1)->point(), face->vertex(2)->point()};
