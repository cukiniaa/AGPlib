 ...
 * \brief Creates an arrangement from points. Points are expected to be vertices of a simple polygon.
...
/*! \fn Arrangement create_arrangement (const std::vector<Kernel::Point_2> &vertices)
 * \brief Creates an arrangement from points. Points are expected to be vertices of a simple polygon.
 * @tparam Arrangement - a class of type CGAL::Arrangement_2
...
/*! \fn std::vector<Polygon_2> arrangements_to_polygons (const std::vector<Arrangement_2> &arrangements)
 * \brief Converts a vector of arrangements to a vector of polygons.
 * @param arrangements - vector of arrangements
 * @return vector of corresponding polygons
 */
std::vector<Polygon_2> arrangements_to_polygons (const std::vector<Arrangement_2> &arrangements);

/*! \fn std::vector<Kernel::Point_2> face_vertices (const Arrangement_2::Face_const_handle &face)
 * \brief Returns a vector of face's vertices.
 * @param face - const handle to a face
 * @return vertices of the given face
 */
std::vector<Kernel::Point_2> face_vertices (const Arrangement_2::Face_const_handle &face);


/*! \fn void print_ccb (typename Arrangement::Ccb_halfedge_const_circulator circ)
 * \brief Prints halfedges of a given connected component boundary.
 * @tparam Arrangement - a class of type CGAL::Arrangement_2
 * @param circ - circulator that allows traversing the halfedges of the connected component boundary
 */
...
/*! \fn Arrangement overlay_arrangements (const Arrangement &primary, const std::vector<Arrangement> &arrangements)
 *  \brief Constructs an overlay of the given arrangement starting from the primary arrangement.
 * @tparam Arrangement - a class of type CGAL::Arrangement_2
 ...
 */
template <typename Arrangement>
Arrangement overlay_arrangements (const Arrangement &primary, const std::vector<Arrangement> &arrangements) {
